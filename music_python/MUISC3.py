...
class MUSICEst:
    ...
    def getSpectrum(self, noiseMat, frequency, noiseMatMode='cov'):
        ...
        spectrum = np.empty(self.angleStepsNum, dtype=np.float64)
    ...
    def getNoiseMat(self, cov, amountOfSources):
        ...
        noiseMat = np.matrix(np.empty((antennasNum, antennasNum - amountOfSources), dtype=np.complex128))
...
def STFT(data, nfft, noOverlap=0):
    ...
    stft = np.empty((nfft // 2 + 1, noWindows), dtype=np.complex128)
...
def IFBMUSICSpectrum(data, micPositions, nfft, amountOfSources=1, angleStepsNum=720,\
    ...
    stft = np.empty((data.shape[0], nfft // 2 + 1, data.shape[1] // nfft), dtype=np.complex128)
    ...
    for binIdx in bins:
        crossSpectrum = np.matrix(np.zeros((stft.shape[0], stft.shape[0]), dtype=np.complex128))
...
if __name__=="__main__":
    ...
    antennaPos = np.array([[-0.0805, 0.0805, 0.0505], [0.0, 0.0, 0.0]])
    angleStepsNum = 720
    searchInterval = (0, 2 * np.pi)
    ...
    data3, sr = librosa.load(folder + "/" + "hal_in_pure_24_4ch_48k_3.wav", sr=None)
  
    print(data1.shape, sr)
    data = np.stack([data1, data2, data3])
    # data = data.astype(np.float)
    print(data.shape)
    mergedSpectrums, angleBins =  IFBMUSICSpectrum(data, 
                               micPositions=antennaPos, 
                               nfft=512, 
                               amountOfSources=1,
                               angleStepsNum=180,
                               searchInterval=(0, np.pi),
                               samplingRate=48000,
                               )
    # print(mergedSpectrums.shape, angleBins.shape)
    max_index = np.argmax(mergedSpectrums)
    max_doa = np.linspace(-90, 90, len(mergedSpectrums))[max_index]
